...
// 테케 1번 틀렸던 코드
// -> dfs 내에서 temp를 깊은 복사를 해버려서
// -> 티켓을 잘못 사용했을 때도 temp를 업데이트해서 오류가 발생했음. 
function solution(tickets) {
  let targetNum = tickets.length+1;
  let answer=[];
  let isFin = false;
  ...
  function dfs (curIdx, temp, ticket, cnt){
      ...
      for(let i=0 ; i<ticket.length; i++){
          ...
          // 해당 티켓의 시작지점이 현재 티켓의 도착지이면
          if(ticket[i][0]===ticket[curIdx][1]){
              console.log('---------', curIdx, cnt, '--------')
              console.log(temp, ticket[i][0]);
              dfs(i, temp, newTicket, cnt);
          }
      }
